        ...
        vpnControlMethod.setMethodCallHandler((call, result) -> {
            switch (call.method) {
                ...
                case "initializeV2Ray":
                    ...
                    V2rayController.init(binding.getApplicationContext(), binding.getApplicationContext().getResources().getIdentifier(iconResourceName, iconResourceType, binding.getApplicationContext().getPackageName()), "Turbo Game");
                    result.success(null);
                    break;
                case "getServerDelay":
                    executor.submit(() -> {
